...
# Public accounts that can be replaced during deduplication
public_accounts = ["Assets:Company:Alipay:StupidAlipay"]
...
class Deduplicate:
    ...
    def find_duplicate(
        self, entry, money, unique_no=None, replace_account="", currency="CNY"
    ):
        # 要查询的是实际付款的账户，而不是支出信息
        bql = "SELECT flag, filename, lineno, location, account, year, month, day, str(entry_meta('timestamp')) as timestamp, metas() as metas WHERE year = {} AND month = {} AND day = {} AND number(convert(units(position), '{}')) = {} ORDER BY timestamp ASC"
        items = query.run_query(
            self.entries,
            self.option_map,
            bql,
            entry.date.year,
        ...
        if length == 0:
            # 精确匹配失败，尝试子集和匹配
            if replace_account != "":
                subset_matched = self.find_subset_sum_match(
                    entry, money, replace_account, currency
                )
                if subset_matched:
                    return True
            # 仍未匹配，记录为未匹配的导入交易
        ...
        for item in items:
            ...
            if (
                ...
                if (
                    "timestamp" in entry.meta
                    and item_timestamp == entry.meta["timestamp"]
                ):
                    break
    ...
    def find_subset_sum_match(
        self, entry, target_amount, replace_account, currency="CNY"
    ):
        """
        ...
        # 过滤掉已经匹配的交易
        available_items = [
            item for item in items if item.location not in _matched_beancount_locations
        ]
        ...
        for idx, (amount, item) in enumerate(int_amounts):
            ...
            for offset in [-1, 0, 1]:
                ...
                if check_target in dp:
                    ...
                    print(
                        f"子集和匹配成功: {target_amount} = {' + '.join(str(a) for a in matched_amounts)}"
                    )
    ...
    def collect_unmatched_beancount(
        self, replace_account, start_date=None, end_date=None, currency="CNY"
    ):
        """
        ...
        bql = """SELECT flag, filename, lineno, location, account, date,
            ...
            currency, replace_account, date_filter
        )
    ...
    def update_transaction_account(self, location, old_account, new_account):
        ...
        print(
            "Updated account from {} to {} at {}".format(
